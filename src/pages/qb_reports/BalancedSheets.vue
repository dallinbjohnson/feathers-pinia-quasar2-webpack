<template>
  ...
  >
      ...
      <q-item>
        <q-item-section top>
          ...
          <q-item-label style="margin-top:-0.2rem; font-size: 7pt" class=" q-mt-none text-bold" caption
            ...
            <div>{{ $lget(reports, ['Header', 'ReportBasis']) }} As of {{
                $lget(reports, ['Header', 'EndPeriod'])
              }}
            </div>
    ...
    <template #page-content>
      ...
      <inbox-dialog v-model="showDatePicker" style="display: flex"
                    :title="`${$lget(reports, ['Header', 'ReportBasis'])} As Of:`">
        <q-card flat square>
          <q-card-section>
    ...
    watch: {
      query: {
        ...
        async handler(newValue) {
          this.fetching = true;
          this.reports = await api.service('quickbooks/reports/balance-sheet').find({
            query: newValue,
          });
          this.fetching = false;
        },
